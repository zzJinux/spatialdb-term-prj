...
struct pqueue create_pq(size_t elem_sz, int (*cmp)(void const *, void const *)) {
    struct pqueue pq;
...
void pq_push(struct pqueue *pq, void *elem) {
    ...
    int i = pq->_arr.len - 1;
    while(i > 1 && pq->_cmp(elem, GET(pq->_arr, i/2)) > 0) {
...
void pq_pop(struct pqueue *pq) {
    ...
    int i = 2;
    while(i < post_arr_sz) {
        if(i+1 < post_arr_sz && pq->_cmp(GET(pq->_arr, i+1), GET(pq->_arr, i)) > 0) {
            ++i;
        }

        if(pq->_cmp(p, GET(pq->_arr, i)) >= 0) break;
